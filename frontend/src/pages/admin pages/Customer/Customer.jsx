...
export default function Customer(){
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://159.65.149.102/api/customer/${id}`);
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get('http://159.65.149.102/api/customer/');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post('http://159.65.149.102/api/customer/register', NewCustomer);
      await axios.post('http://159.65.149.102/api/users/register', NewUser);
